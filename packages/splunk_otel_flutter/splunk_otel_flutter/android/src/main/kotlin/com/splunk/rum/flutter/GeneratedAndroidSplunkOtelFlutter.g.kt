...
interface SplunkOtelFlutterHostApi {
  ...
  fun stateSetEndpointConfiguration(endpointConfiguration: GeneratedEndpointConfiguration, callback: (Result<Unit>) -> Unit)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: SplunkOtelFlutterHostApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.splunk_otel_flutter_platform_interface.SplunkOtelFlutterHostApi.stateSetEndpointConfiguration$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val endpointConfigurationArg = args[0] as GeneratedEndpointConfiguration
            api.stateSetEndpointConfiguration(endpointConfigurationArg) { result: Result<Unit> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(GeneratedAndroidSplunkOtelFlutterPigeonUtils.wrapError(error))
              } else {
