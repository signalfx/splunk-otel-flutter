...
1. **Locate the script:**
   - **Flutter apps:** use the script bundled with the plugin (no copy needed)
   - **Native iOS apps:** copy `upload-dsyms.sh` into your Xcode project directory and add it to the project (but not any target)
   - Ensure the script is executable (`chmod +x`) or invoke it with `bash`

2. **Set up the build phase:**
   - In Xcode, select your project in the navigator
   - Select your app target (Flutter: open `ios/Runner.xcworkspace` and select the `Runner` target)
...
# Flutter plugin path (Runner target). Adjust if your plugin path differs.
SCRIPT_PATH="${SRCROOT}/../.symlinks/plugins/splunk_otel_flutter/dsymUploader/upload-dsyms.sh"

# Native iOS example:
# SCRIPT_PATH="${SRCROOT}/Scripts/upload-dsyms.sh"
...
if [[ -f "$SCRIPT_PATH" ]]; then
    ...
    # Set SPLUNK_REALM and SPLUNK_API_ACCESS_TOKEN in Build Settings or replace here.
    /bin/bash "$SCRIPT_PATH" \
        --realm "${SPLUNK_REALM}" \
        --token "${SPLUNK_API_ACCESS_TOKEN}" \
    ...
    echo "Error: Splunk dSYM upload script not found at: $SCRIPT_PATH"
    exit 1 # Fail the Xcode build if the script itself is missing
fi
```
   
4. **Set your configuration (recommended)**:

   - **Build Settings / Environment Variables:**
        Set `SPLUNK_REALM` and `SPLUNK_API_ACCESS_TOKEN` in your Xcode scheme or CI. `DWARF_DSYM_FOLDER_PATH` is provided by Xcode.

   - **Manual runs (Environment Variables):**
   ```bash
...
#### Script Not Found
```
Warning: Splunk dSYM upload script not found
```
- Verify the script path in your build phase
- For Flutter apps, verify the plugin path under `ios/.symlinks/plugins/splunk_otel_flutter/dsymUploader/upload-dsyms.sh`
...
5. File an issue in the [splunk-otel-flutter](https://github.com/signalfx/splunk-otel-flutter) repository
