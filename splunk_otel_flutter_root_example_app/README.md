...
4. **Recording mask** - Sets a `RecordingMask` with two elements: a `covering` mask (200x100 at origin) and an `erasing` mask (100x50 at offset 50,50). Then reads it back to verify the round-trip through the Pigeon bridge.
